#!/usr/bin/env python3
"""
ITERACIÓN 24: ESTRATEGIA GANADORA EN TIMEFRAME 5M
===================================================================

CONTEXTO:
- Iteración 22: PF 2.18 (éxito técnico) pero solo 32 trades en 15m
- Iteración 23: Intentó alta frecuencia en 15m → FALLÓ
- Conclusión: No podemos alcanzar frecuencia de Day Trading en 15m

HIPÓTESIS V24:
Al aplicar la lógica ganadora de Donchian/EMA (que funcionó en v22) en un
timeframe de 5 minutos, multiplicaremos la frecuencia de trades para alcanzar
nuestro objetivo (> 150 trades) mientras mantenemos la rentabilidad (PF > 1.2).
...
- Lógica de señales (v18/v22):
  * COMPRA = Breakout Arriba + Precio > EMA
  * VENTA = Breakout Abajo + Precio < EMA

PARÁMETROS A OPTIMIZAR (Grid Search):
- donchian_period: [10, 15, 20] (Períodos que casi funcionaron en 15m)
- ema_filter_period: [50, 100, 150] (Filtros que funcionaron)
- sl_multiplier: [3.0, 4.0] (Stop Loss)
- tp_multiplier: [3.0, 4.0] (Take Profit)

Total de combinaciones: 3 × 3 × 2 × 2 = 36 configuraciones
...
DATOS:
- Activo: ETHUSDT
- Timeframe: 5m (clave para alta frecuencia)
...
# Configurar logger
logger = setup_logger("phase2_optimize_v24", "logs/phase2_optimize_v24.log")


def main():
    logger.info("=" * 80)
    logger.info("ITERACIÓN 24: ESTRATEGIA GANADORA EN TIMEFRAME 5M")
    logger.info("=" * 80)
    logger.info("\nHipótesis: Lógica ganadora (Donchian/EMA) en 5m → Alta frecuencia + Rentabilidad")
    ...
    # =========================================================================
    # 2. DEFINIR GRID DE PARÁMETROS (LÓGICA GANADORA)
    # =========================================================================
    logger.info("\n2. Definiendo Grid de Parámetros (Lógica Ganadora v22)...")

    param_grid = {
        'donchian_period': [10, 15, 20],      # Períodos que casi funcionaron
        'ema_filter_period': [50, 100, 150],  # Filtros de tendencia que funcionaron
        'sl_multiplier': [3.0, 4.0],          # Stop Loss en múltiplos de ATR
        'tp_multiplier': [3.0, 4.0]           # Take Profit en múltiplos de ATR
    }
    ...
    output_file = project_root / "backtest_results_eth_v24_donchian_5m.csv"
    ...
    if len(filtered) > 0:
        ...
        logger.info("\n✓✓✓ ITERACIÓN 24 APROBADA ✓✓✓")
        logger.info("¡ÉXITO! Encontramos configuraciones que cumplen ambos objetivos:")
        logger.info("  1. Alta frecuencia Day Trading (> 150 trades)")
        logger.info("  2. Rentabilidad robusta (PF > 1.2)")
        logger.info("\nEsta es la estrategia ganadora para proceder a live trading.")

    else:
        logger.info("\n✗✗✗ ITERACIÓN 24 SIN APROBACIÓN ✗✗✗")
    ...
    logger.info("     - Considerar ajustar objetivo de frecuencia")
    logger.info("     - Evaluar timeframe 1m como última opción")
